...
- [ ] **`Profile.resumeFile` not set.** No resume file has been uploaded to Storage (`06` §3.2), and `§3` below hasn't settled PDF-only vs inline preview. The field is optional so the site renders without a resume link rather than a broken one — but `02` §3 and brief §28 put Resume in both header and footer as a top-priority recruiter exit path, so this is worth closing before launch.
