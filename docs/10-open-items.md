...
- [ ] **`Profile` cannot be seeded at all.** `heroStatement` is still unlocked (§2 above), `bioShort` / `bioLong` have never been written, and **no contact details exist anywhere in `00`–`10`** — no email, no LinkedIn URL, no GitHub URL, no Instagram or Facebook URL. `02` §3 puts Email/LinkedIn/GitHub in the footer of every page, so this blocks the Hero, the Contact page and the footer. **This is now the last thing blocking Phase 3.**
...
- [~] **Skill *levels* — narrowed (2026-08-27), not closed.** brief §11 was updated to split "Expanding Toward" into two stated tiers, and those map exactly onto `04` §5's bottom two levels: **Learning** (C#, .NET, SQL Server Databases — "course basics completed, not yet applied in a real project") and **Interested in** (Node.js, AI-assisted development tools — "clear next step, not yet started"). Both are seeded as that literal split; collapsing them back into one tier would erase a distinction the brief makes deliberately. **Still open:** the brief draws no line between `strong` and `good` inside "Current technical focus", so all 34 of those skills seed as `good` — a floor the source supports. One editorial pass needed before the Skills section publishes; guessing `strong` would assert a proficiency Muhammed hasn't asserted (`01` §9, Rule 4).
